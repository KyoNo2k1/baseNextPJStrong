...
export default function useHeaderSearch<T>(_onChangeSearchParams: (value: any) => void) {
  const [searchText, setSearchText] = useState('');
  const [searchedColumn, setSearchedColumn] = useState('');
  const searchInput = useRef<InputRef>(null);
  const { trans, dateLang } = useTrans()
  const getColumnSearchProps = ({ dataIndex, isDate, formatFunc }: { dataIndex: keyof T, isDate?: boolean,
    formatFunc?: (...params: any) => string, fieldType: any, operation: any }): ColumnType<T> => ({
    filterDropdown: ({ setSelectedKeys, selectedKeys, confirm, close, filters, }) => (
      <div style={{ padding: 8 }} onKeyDown={(e) => e.stopPropagation()}>
        ...
        {isDate &&
          <DatePicker  locale={dateLang} style={{ marginBottom: 8, display: 'block' }}
            placeholder={`${trans.common.search} ${String(dataIndex)}`}
